...
public class SchedulerTest {
	...
	//@Scheduled(fixedDelay = 3000) // 메서드 종료 후 5초 후에 다시 메서드 실행
	//@Scheduled(fixedRate = 3000) // 메서드 실행 시점으로 부터 5초후에 다시 메서드 실행, ms단위
	@Scheduled(cron = "0 * * * * *") // 매 분 0초마다 실행
	@Scheduled(cron = "0 0 0 * * *") // 자정마다 실행 ex) 하루 누적 방문자 수 
	public void test1() {
//		System.out.println("3초마다 실행중...");
		System.out.println("매 분 0초마다 실행 중....");
	}
