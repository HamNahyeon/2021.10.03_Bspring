...
<body>
			...
			<div class="col-md-6 offset-md-3">
				...
				<form method="POST" action="signUp" class="needs-validation" name="signUpForm" onsubmit="return validate();">
					...
					<div class="row mb-5 form-row">
						...
						<!-- ajax를 이용한 중복검사로 입력할 때마다 바로바로 검사가 되기 때문에
							위아래 구문이 필요가 없어짐 
							<input type="hidden" name="idDup" id="idDup" value="false"> 
	...
	<script>
		...
		$("#idDupCheck").on("click", function(){
			// window.open("주소/경로", "창 이름", "스타일설정"); 
			// -> 팝업창의 주소, 여러 팝업창을 구분할 수 있는 이름, 팝업창의 위치 크기 등 스타일 (px단위)
			window.open("idDupCheck", "idDupForm", "width=450, height=250");
			// 절대경로 : ${contextPath}/member/idDupCheck
			// 상대경로 : idDupCheck
			// -> 상대경로는 현재 위치 기준: 짧지만 해석이 어려움 /* http://localhost:8080/semi/ 뒤에오는 주소만 바뀜 */
			// 절대경로는 명확하지만 너무 김
