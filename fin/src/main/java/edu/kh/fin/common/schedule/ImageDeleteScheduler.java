...
public class ImageDeleteScheduler {
	...
	//		 DB  : A B
	//		 -> 서버에 C, D삭제
	// 서버에 있는 애들 가져오기 DB에 있는 애들 가져오기 두개 비교하기
	
	@Autowired
	private BoardService service;
	
	@Autowired
	private ServletContext servletContext; // 서버경로를 얻어오기 위한 객체
	
//	@Scheduled(cron="0 0 0, 12 * * *") // 매일 0시, 12시 마다 실행
	@Scheduled(cron="0 * * * * *") // 매 분 30초마다 실행
	public void deleteImage() {
		...
		String standard = sdf.format(threeDaysAgo); // 72시간 전 시간이 yyyyMMddHH 형식의 문자열로 저장
		
		// 72시간보다 더 이 전에 생성된 파일을 저장할 List생성
		...
		for(File f : serverFileList) {
			...
			// C:\workspace\8_Framework\fin\src\main\webapp\resources\images\freeboard\20210730153558_85176.jpg
			// System.out.println(f.toString());
			
			// 경로를 제거해서 파일 이름만 저장하기
			// String fileName = f.toString().substring( f.toString().lastIndexOf("\\") + 1 );
			// System.out.println(fileName);
			// String,subString(index) : 문자열을 index부터 끝까지 부분만을 잘라내어 반환
			// String,subString(index) : 문자열을 뒤에서 부터 검색하여 str가 일치하는 부분의 index반환
		...
		// for문 종료 후 list에는 72시간 보다 과거에 생성된 파일만 담겨 있음.
		...
		System.out.println("-------------------------------------------------");
		  ...
		  // for(String str : dbList) { System.out.println(str); }
		 
		
		// list : 72시간 보다 과거에 추가된 서버 파일 목록
		// dbList : 72시간 보다 과거에 추가된 DB파일명 목록
		...
		if( !list.isEmpty() && !dbList.isEmpty()) { // 두 list에 데이터가 있을 경우 
			for(File f : list) {
				if(dbList.indexOf(f.getName()) < 0) {
					...
					// List.indexOf("객체") : List내부에 "객체"와 일치하는 객체가 존재하면
					//								해당 인덱스를 반환, 없으면 -1반환
